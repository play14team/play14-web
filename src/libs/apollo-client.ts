...
export async function query<TQuery, TQueryVariables>({ query, variables }: QueryParameters<TQuery, TQueryVariables>) {
...
export function attributesAs<TEntity>(result: any) {
	...
	if (!attributes) throw new Error("Query response does not return expected attributes")
